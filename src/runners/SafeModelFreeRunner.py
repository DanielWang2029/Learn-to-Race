...
from src.runners import ModelFreeRunner
from src.runners.base import BaseRunner
from src.utils.envwrapper_safety import SafeEnvContainer
...
@yamlize
class SafeModelFreeRunner(BaseRunner):
    ...
    def __init__(
        ...
        if use_container:
            self.env_wrapped = SafeEnvContainer(self.encoder)
        else:
    ...
    def run(self, env, api_key: str = ""):
        ...
        for ep_number in range(start_idx + 1, self.num_run_episodes + 1):
            ...
            while not done:
                ...
                # Penalize usage of safety system
                if self.agent.record['transition_actor'] == 'safepol':
                    self.replay_buffer.penalize_most_recent_transition(min(50, max(3, 5 * max(obs_encoded.flatten()[-4].item()-3, 0))))
