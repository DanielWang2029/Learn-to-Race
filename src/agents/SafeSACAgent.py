...
class SafeSACAgent(SACAgent):
    ...
    def select_action(self, feat, nearest_idx = -1, deterministic=False):
        speed = feat.flatten()[-4].item() # in m/s
        safety_value, local_state = self._get_safety_value(feat, nearest_idx) # Send in IMU data
        
        if (safety_value <= (0 + self.safety_margin)) & (self.t > self.steps_to_sample_randomly):
            a = self.safety_controller.select_action(local_state, self.safety)
            ## Penalize the action that activates safe controller
                
            # TODO: Add intervention logging in a better way...
            ...
            if (self.t <= self.steps_to_sample_randomly):
                a = np.random.uniform([-1, -0.125], [1, 1]) # Why are there so many specific constants??
                self.record['transition_actor'] = 'random'
            else:
                a = self.actor_critic.act(feat, self.deterministic or deterministic)
                self.record['transition_actor'] = 'learner'
                # More specific stuff
                a[1] = np.clip(a[1], a_min = -0.125, a_max = 1)   
                
        ## Prevent the car from being stuck (business logic should be cleaner.)
        if speed < 0.3:
            a[1] = max(a[1], 0.1)   
        elif speed < 1:
            a[1] = max(a[1], 0)
