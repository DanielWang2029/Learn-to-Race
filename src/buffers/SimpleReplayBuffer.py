...
class SimpleReplayBuffer:
    ...
    def store(self, buffer_dict):
        ...
        def convert(arraylike):
            ...
            if isinstance(obs, torch.Tensor):
                ...
                obs = obs.cpu().numpy()
            return obs
        self.obs_buf[self.ptr] = convert(buffer_dict["obs"])[:,:self.obs_buf.shape[1]]
        self.obs2_buf[self.ptr] = convert(buffer_dict["next_obs"])[:,:self.obs_buf.shape[1]]
    ...
    def penalize_most_recent_transition(self, penalty):
        self.rew_buf[max(0, self.ptr-1)] = -penalty
